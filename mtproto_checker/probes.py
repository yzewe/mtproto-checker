...
from .constants import (
    MTTRANSPORT_ABRIDGED,
    MTTRANSPORT_INTERMEDIATE,
...
def _validate_fake_tls_server_hello_shape(server_hello: bytes) -> None:
    try:
        records = _iter_tls_records(server_hello)
    except ValueError as exc:
        raise ConnectionError("invalid FakeTLS ServerHello records") from exc
...
async def _check_fake_tls_mtproto_target(
    ...
    transport_candidates = [
        ...
        (
            "faketls-intermediate",
            "modern",
            MTTRANSPORT_INTERMEDIATE,
            _encode_intermediate_packet,
            _read_intermediate_packet,
        ),
        (
            "faketls-abridged",
            "modern",
    ...
    for dc_id in TELEGRAM_DC_IDS:
        for transport_name, hello_style, protocol, encoder, decoder in transport_candidates:
            reader = None
            writer = None
            try:
                reader, writer = await asyncio.wait_for(
...
async def _check_fake_tls_mtproto(
    ...
    *,
    ...
    hello_style: str = "modern",
) -> None:
    if hello_style == "legacy":
        client_hello = _make_legacy_fake_tls_client_hello(_secret_sni(secret))
        session_id = None
        client_random = None
    else:
    ...
    if hello_style == "legacy":
        _validate_fake_tls_server_hello_shape(server_hello)
    else:
...
async def _read_direct_abridged_packet(
    ...
    length = length_words * 4
    if length <= 0 or length > 1_048_576:
        raise ConnectionError(f"invalid direct abridged packet length: {length}")
    return await _read_exactly(reader, length, timeout)
